...
export class CryptoService {
  ...
  async getStakingProducts(): Promise<StakingProduct[]> {
    // Mock staking products
    return [
      {
        id: "eth-flexible-staking",
        asset: "ETH",
        type: "FLEXIBLE",
        apy: 4.5,
        minAmount: 0.1,
        ...
        risks: ["Medium", "Smart Contract Risk"],
        features: ["No Lock-up", "Daily Rewards", "Instant Withdrawal"]
      },
      {
        id: "btc-locked-staking",
        asset: "BTC",
        type: "LOCKED",
        apy: 6.2,
        minAmount: 0.01,
        ...
        isActive: true,
        totalStaked: 890.2,
        description: "90-day locked Bitcoin staking with higher rewards",
        risks: ["High", "Lock-up Period", "Smart Contract Risk"],
        features: ["High APY", "Locked 90 Days", "Compound Rewards"]
      }
    ];
  }
  ...
  async getStakingPositions(): Promise<StakingPosition[]> {
    ...
    return [
      {
        id: "stake-1",
        productId: "eth-flexible-staking",
        asset: "ETH",
        amount: 2.5,
        rewardAsset: "ETH",
        apy: 4.5,
  ...
  // Stake asset
  async stakeAsset(productId: string, amount: number): Promise<StakingPosition> {
    // Mock staking position creation
    const products = await this.getStakingProducts();
    const product = products.find(p => p.id === productId);
    
    if (!product) {
      throw new Error("Staking product not found");
    }
    
    // Calculate daily reward
    const dailyReward = (amount * product.apy) / 100 / 365;
